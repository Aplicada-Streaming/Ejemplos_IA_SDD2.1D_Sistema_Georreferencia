...
La cadena de generación incluye una categoría 10 (developer guide) cuyo destinatario es un desarrollador externo o integrador que consume el proyecto desde afuera. Para el tipo de este proyecto esa categoría es opcional, no obligatoria: la audiencia de consumo es interna al equipo y a la operación, no existe un desarrollador integrador externo que necesite un onboarding propio, el proyecto no publica un kit de desarrollo ni un canal de extensión hacia terceros y no expone un portal de developers (la bandera de portal de developers está en false en el manifiesto de la solución). En ausencia de una superficie pública de integración, la documentación orientada al consumidor se reduce a lo que un miembro del equipo o de operación necesita para instalar y operar el proyecto, y ese alcance se cubre con el README del repositorio. La regla de la categoría 10 establece que cuando la categoría es opcional y el equipo decide omitirla, la decisión se registra en una ADR con su justificación; la orquestación, para la fase de developer guide del proyecto, indica que si la categoría se omite se debe registrar un ADR de omisión en la categoría 05 del proyecto. Esta ADR materializa ese registro.
...
Aceptado el 2026-06-15. La omisión es una decisión firme del arquitecto fundada en el tipo del proyecto y en las banderas del manifiesto (audiencia interna, sin superficie pública de integración, sin portal de developers). Si en una evolución futura el proyecto expusiera una superficie de integración hacia terceros (un kit de desarrollo, un canal de extensión o un portal de developers), esta decisión dejaría de aplicar: se crearía una ADR nueva con identificador siguiente que habilite la categoría 10 y esta pasaría a `Superado por ADR-YY` sin reescribirse.
...
- La decisión se valida con que el README de la solución y el README de la sección cubran el consumo interno: instalación, ejecución y operación del proyecto por un miembro del equipo o de operación sin necesidad de una developer guide separada.
...
- Reglas de la categoría 10, §1.2 y §2.2: la categoría es opcional para este tipo; la audiencia es el equipo interno y la operación, y la guía suele colapsar en el README del repositorio; se materializa solo si el proyecto expone una superficie pública de integración.
