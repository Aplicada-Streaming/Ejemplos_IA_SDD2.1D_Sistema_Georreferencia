...
Consecuencia de no construirlo y disparador externo (por qué ahora): PENDIENTE.

## §2 Audiencia y stakeholders

La jerarquía de usuarios quedó definida en cuatro niveles, de mayor a menor alcance: usuario raíz → jefe general → jefe de área → agente de campo. Faltan los stakeholders de propiedad e implementación.

| Rol | Nombre o cargo | Categoría | Responsabilidad principal |
|---|---|---|---|
| Dueño del problema / aprobador del intake | PENDIENTE | Propietario | Aprueba el intake |
| Equipo de desarrollo | PENDIENTE | Implementador | Construye y mantiene la solución |
...
No se determinaron métricas de negocio en la conversación de descubrimiento.
...
- Fecha objetivo y qué la motiva: PENDIENTE.
- Presupuesto orientativo o rango: PENDIENTE.
- Restricciones legales o regulatorias: PENDIENTE.
- Integraciones obligatorias con sistemas existentes: PENDIENTE.
...
No se relevaron riesgos de negocio con su probabilidad, impacto y mitigación en la conversación.
...
- `geovial-mobile` (`mobile-app-maui`): samples de app básica, sync offline y, si aplica el target, multiplataforma (ver §17 P.9 del proyecto; multiplataforma PENDIENTE si solo se soporta Android).
...
#### §17.P.9 Compatibilidad y plataformas target
Android (dispositivo de desarrollo por USB). Versión mínima de Android y soporte de otras plataformas (iOS/Windows): PENDIENTE.
...
#### §17.P.7 Estrategia de versionado y release
Se adopta SemVer 2.0.0 y Conventional Commits. No se publica como NuGet (se integra al backend). Herramienta de versionado: PENDIENTE.

#### §17.P.8 Pipeline CI/CD
Se construye junto al backend mediante los scripts `.bat`. Quality gates: PENDIENTE.

#### §17.P.9 Compatibilidad y plataformas target
Runtime .NET del backend. Versión mínima: PENDIENTE.

#### §17.P.10 Requerimientos no funcionales (NFR)
Métricas (latencia de subida/descarga, tamaño máximo de archivo): PENDIENTE.
...
#### §17.P.7 Estrategia de versionado y release
Se adopta SemVer 2.0.0 y Conventional Commits. Se publica como paquete NuGet redistribuible con repositorio en GitHub. Feed exacto (por ejemplo GitHub Packages o nuget.org) y herramienta de versionado: PENDIENTE.

#### §17.P.8 Pipeline CI/CD
CI que construye y publica el paquete NuGet desde GitHub. Stages, quality gates bloqueantes y verificación post-publish: PENDIENTE.

#### §17.P.9 Compatibilidad y plataformas target
Plataformas MAUI (al menos Android, en línea con la app móvil). Versiones mínimas y otras plataformas soportadas: PENDIENTE.

#### §17.P.10 Requerimientos no funcionales (NFR)
Métricas (tiempos de sync, tolerancia a cortes, tamaño de lote): PENDIENTE.
...
Negocio (Parte A):
- [x] La cabecera tiene nombre de solución, cliente, fecha y estado. (cliente y lead PENDIENTE, pero los campos están y el documento declara su estado)
- [x] §1 describe un problema concreto y qué pasa si no se construye. (problema sí; "qué pasa si no se construye" PENDIENTE)
- [ ] §2 tiene al menos un stakeholder por categoría con rol explícito. (faltan propietario e implementador)
...
- [ ] §9 lista al menos 3 exclusiones con justificación. (1 de 3)
