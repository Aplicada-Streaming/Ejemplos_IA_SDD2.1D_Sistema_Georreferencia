...
.NET / C#, ASP.NET Core Web API. Dependencias core: acceso a datos sobre SQL Server e integración de la librería `GeoVial.Storage` para fotos. Versión mínima de .NET y runtime (propuesto, ratificable): .NET 8 (LTS). Dependencias core (propuesto, ratificable): ORM con migraciones versionadas del runtime y driver de SQL Server; autenticación con tokens JWT bearer.

#### §17.P.2 Estilo arquitectónico del proyecto
Monolito de backend (fijado por requisito). Alternativas descartadas: microservicios (descartado por el requisito de monolito) y servir la UI desde el mismo proceso (descartado: el front es un proyecto Blazor separado). Estilo interno (propuesto, ratificable): Clean Architecture en capas (Dominio, Aplicación, Infraestructura, API).

#### §17.P.3 Comunicación e integración
REST sobre HTTP, payloads JSON, autenticación JWT bearer con flujo ROPC. Expone a `geovial-web` y `geovial-mobile`; incluye los endpoints de sincronización (recibe primero los cambios locales y luego entrega actualizaciones). La sincronización admite estados en conflicto (marcadores dentro de un mismo radio) como válidos: no bloquea la integración y difiere la resolución al cierre del relevamiento. Versionado de contratos (propuesto, ratificable): versionado por URI (prefijo de versión mayor en la ruta) con política de breaking changes que mantiene la versión previa al menos un MINOR antes de removerla.

#### §17.P.4 Persistencia
SQL Server (fijado por requisito). El modelo persiste marcadores en conflicto sin forzar su unificación durante la recolección; la unificación/separación se aplica al cierre. Versionado del esquema (propuesto, ratificable): migraciones versionadas de la herramienta del ORM, ejecutadas en el arranque controlado del despliegue. Multi-tenant (propuesto, ratificable): no; la solución es single-tenant para una única organización (Vialidad provincial) y la jerarquía de cuatro roles es control de acceso, no aislamiento por tenant. Define el flag multi_tenant = false.

#### §17.P.5 Seguridad y autenticación
Autenticación y autorización por flujo ROPC con bearer token JWT (fijado por requisito). Roles: raíz, jefe general, jefe de área y agente de campo, con el alcance jerárquico de §2. Identity Provider (propuesto, ratificable): el propio backend emite y valida el JWT; no hay IdP externo. Secretos en un secret manager o vault, nunca en commit. Compliance: sin exigencias regulatorias (intake §10).

#### §17.P.6 Estrategia de testing
Se prevén pruebas unitarias del manejo de usuarios y la administración por jerarquía. Cobertura mínima (gate de CI), propuesto (ratificable): líneas >= 80 %, branches >= 70 %; capa de aplicación >= 80 %, infraestructura >= 70 %, y 100 % de los endpoints públicos cubiertos por un test de contrato. Frameworks (propuesto, ratificable): pruebas unitarias, pruebas de integración contra una base de datos efímera y pruebas de contrato de la API.

#### §17.P.7 Estrategia de versionado y release
Se adopta SemVer 2.0.0 y Conventional Commits. Herramienta y release (propuesto, ratificable): versión derivada del tag con GitVersion; branching trunk-based (equipo_n=1); el artefacto publicable es una imagen de contenedor (no un paquete); promoción por ambientes DEV/QA/STAGING/PROD.

#### §17.P.8 Pipeline CI/CD
Existen scripts `.bat` (`build-*.bat`, `publish-*.bat`) y scripts de creación de imágenes de contenedor. Plataforma y gates (propuesto, ratificable): CI con stages lint, build, pruebas (unitarias/integración/contrato), SCA, SBOM, firma y publicación de la imagen. Quality gates bloqueantes para mergear: compilación sin warnings tratados como error, pruebas en verde, gate de cobertura y análisis estático sin issues críticos. Rollback: redepliegue de la imagen previa con desvío de tráfico.

#### §17.P.9 Compatibilidad y plataformas target
Se ejecuta en un contenedor de backend dedicado. Runtime/SO base (propuesto, ratificable): contenedor Linux con runtime .NET 8 (LTS), imagen base de ASP.NET Core 8; versión mínima .NET 8.

#### §17.P.10 Requerimientos no funcionales (NFR)
Métricas (propuesto, ratificable): latencia p95 de lecturas <= 300 ms y de escrituras <= 500 ms en ambiente equivalente al productivo; disponibilidad objetivo 99,5 % mensual; el endpoint de sincronización tolera lotes de >= 1000 cambios por relevamiento. Sin SLO de disponibilidad >= 99,9 % ni objetivo de latencia p99 numérico; define el flag tiene_observabilidad_critica = false.
...
#### §17.P.4 Persistencia
No aplica como almacén de dominio: el estado de dominio se persiste vía la API; el front no tiene base de datos propia. Estado de UI/caché (propuesto, ratificable): solo estado efímero del circuito interactivo y caché en memoria de la sesión; sin persistencia propia. Define el flag tiene_persistencia = false.

#### §17.P.5 Seguridad y autenticación
Obtiene el token del backend enviando credenciales y lo usa como token bearer para las llamadas a la API. Manejo de token y secretos (propuesto, ratificable): el token se mantiene del lado del servidor asociado al circuito interactivo (no se expone al navegador) y los secretos viven en un secret manager o vault, nunca en commit.

#### §17.P.6 Estrategia de testing
Cobertura mínima (gate de CI), propuesto (ratificable): líneas >= 80 %, branches >= 70 %; capa de aplicación >= 80 %, infraestructura >= 70 %, presentación >= 60 %. Frameworks: pruebas unitarias, de integración contra una base efímera (a través de la API), de componente de interfaz con un motor headless y snapshot de vistas clave.

#### §17.P.7 Estrategia de versionado y release
Se adopta SemVer 2.0.0 y Conventional Commits. Herramienta y release (propuesto, ratificable): versión derivada del tag con GitVersion; branching trunk-based (equipo_n=1); artefacto publicable = imagen de contenedor del front; promoción por ambientes DEV/QA/STAGING/PROD.

#### §17.P.8 Pipeline CI/CD
Scripts `.bat` y de imagen de contenedor del front. Plataforma y gates (propuesto, ratificable): CI con stages lint, build, pruebas (unitarias/integración/componente/snapshot), SCA, SBOM, firma y publicación de la imagen; quality gates bloqueantes (compilación sin warnings tratados como error, pruebas en verde, gate de cobertura, análisis estático sin issues críticos); rollback por redepliegue de la imagen previa.

#### §17.P.9 Compatibilidad y plataformas target
Se ejecuta en un contenedor de front dedicado. Runtime/plataformas (propuesto, ratificable): contenedor Linux con runtime .NET 8 (LTS); navegadores evergreen de escritorio y móvil (últimas dos versiones mayores de los navegadores de uso corriente); versión mínima .NET 8.

#### §17.P.10 Requerimientos no funcionales (NFR)
Métricas (propuesto, ratificable): latencia de interacción p95 <= 200 ms sobre el circuito en red estable; soporta al menos 50 circuitos interactivos concurrentes en el ambiente de referencia; disponibilidad objetivo 99,5 % mensual. Sin SLO >= 99,9 % ni p99 numérico; define el flag tiene_observabilidad_critica = false.
...
#### §17.P.4 Persistencia
SQLite local (fijado por requisito) para trabajar sin conexión. Versionado del esquema local y metadatos de sincronización (propuesto, ratificable): migraciones versionadas del esquema local ejecutadas en el arranque de la app; los metadatos de sincronización los gestiona la librería de sincronización (`Aplicada.Sync`). Define el flag tiene_persistencia = true.

#### §17.P.5 Seguridad y autenticación
La primera vez, el usuario inicia sesión online ingresando sus credenciales y recibe un token bearer. Existe un deslogueo completo que libera el dispositivo para que otro usuario lo use con su propia cuenta. Durante una sesión activa, si el dispositivo se bloqueó o la aplicación se reinició, la app pide reloguearse mediante la seguridad del propio dispositivo (por ejemplo, patrón o huella digital), sin volver a pedir credenciales. Almacenamiento del token (propuesto, ratificable): en el almacén seguro del dispositivo provisto por la plataforma; nunca en texto plano.

#### §17.P.6 Estrategia de testing
Cobertura mínima (gate de CI), propuesto (ratificable): líneas >= 80 %, branches >= 70 %; capa de lógica >= 75 %, presentación >= 60 %. Frameworks: pruebas unitarias de la lógica, pruebas de interfaz móvil de las pantallas críticas y pruebas del modo offline/sincronización (cola, reanudación, convivencia con conflicto).

#### §17.P.7 Estrategia de versionado y release
Se adopta SemVer 2.0.0 y Conventional Commits. Herramienta y canales (propuesto, ratificable): versión derivada del tag con GitVersion; canales internal/alpha/beta/production; distribución del paquete de aplicación Android (APK/AAB) por un canal de distribución interno (no se publica en tienda pública en v1).

#### §17.P.8 Pipeline CI/CD
La depuración y prueba se hace sobre un dispositivo Android conectado por USB en modo desarrollador. Plataforma y gates (propuesto, ratificable): CI que compila la app, corre las pruebas, aplica los quality gates bloqueantes (compilación, pruebas en verde, gate de cobertura, análisis estático), firma el paquete con un keystore resguardado y lo publica al canal de distribución interno. Rollback: redistribución de la versión previa por el mismo canal.

#### §17.P.9 Compatibilidad y plataformas target
Android únicamente; no se soportan iOS ni Windows en v1 (decisión confirmada por el cliente). Depuración sobre dispositivo Android conectado por USB en modo desarrollador. Target/versión mínima (propuesto, ratificable): target framework net8.0-android sobre .NET 8 (LTS); versión mínima de Android API 26 (Android 8.0).

#### §17.P.10 Requerimientos no funcionales (NFR)
Métricas (propuesto, ratificable): la captura de una observación con foto funciona 100 % sin conexión; la cola local tolera >= 1000 cambios pendientes; un ciclo de sincronización de 100 cambios completa <= 30 s en red móvil típica y reanuda sin pérdida tras un corte; arranque en frío de la app <= 3 s en el dispositivo de referencia. Sin SLO >= 99,9 % ni p99 numérico; define el flag tiene_observabilidad_critica = false.
...
| 1.4 | 2026-06-15 | Resolución de la Parte C diferida del proyecto de nivel 1 `geovial-api` (rest-api, principal) con defaults ratificables: §17 P.1 (.NET 8 LTS), P.2 (Clean Architecture en capas), P.3 (versionado de contrato por URI), P.4 (migraciones versionadas; multi_tenant = false), P.5 (el backend como IdP; secretos en vault; sin compliance), P.6 (cobertura >= 80/70 y 100 % de endpoints con contract test), P.7 (GitVersion, trunk-based, imagen de contenedor, DEV/QA/STAGING/PROD), P.8 (CI con gates bloqueantes y rollback por redepliegue), P.9 (.NET 8 LTS, contenedor Linux), P.10 (latencias p95, disponibilidad 99,5 %; tiene_observabilidad_critica = false). Quedan diferidas las Partes C de geovial-web y geovial-mobile. Snapshot v1.3 en `_legacy/2026-06-15/`. | Orquestador SDD 2.1 |
| 1.5 | 2026-06-15 | Resolución de la Parte C diferida de los proyectos de nivel 2 con defaults ratificables, completando los 5 proyectos. geovial-web (web-monolith): §17 P.4 (sin persistencia propia; tiene_persistencia = false), P.5 (token del lado servidor del circuito; secretos en vault), P.6 (cobertura 80/70 + presentación 60), P.7/P.8 (GitVersion, imagen de contenedor, DEV/QA/STAGING/PROD, gates), P.9 (.NET 8 LTS, navegadores evergreen), P.10 (interacción p95 <= 200 ms, >= 50 circuitos, 99,5 %; tiene_observabilidad_critica = false). geovial-mobile (mobile-app-maui): §17 P.4 (migraciones locales; metadatos de sync por la librería; tiene_persistencia = true), P.5 (token en almacén seguro del dispositivo), P.6 (cobertura 80/70 + lógica 75 / presentación 60), P.7 (canales internal/alpha/beta/production, distribución interna del paquete Android), P.8 (CI con firma por keystore y gates), P.9 (net8.0-android, Android API mínima 26), P.10 (captura 100 % offline, cola >= 1000, sync 100 cambios <= 30 s, arranque <= 3 s; tiene_observabilidad_critica = false). Snapshot v1.4 en `_legacy/2026-06-15/`. | Orquestador SDD 2.1 |
